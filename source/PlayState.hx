...
class PlayState extends MusicBeatState
{
	...
	var blackback:FlxTypedGroup<FlxSprite>;
				...
				case 'heaven1':
					{
						...
						var bgCircle:FlxSprite = new FlxSprite(-5, -5);
					    bgCircle.frames = Paths.getSparrowAtlas('funkinHeaven/square');
					    bgCircle.animation.addByPrefix('drive', "square", 20);
					    bgCircle.animation.play('drive');
					    bgCircle.scrollFactor.set(0.1, 0.1);
					    add(bgCircle);
					    bgCircle.updateHitbox();
		                bgCircle.screenCenter();
						
						blackback = new FlxTypedGroup<FlxSprite>();
						...
						for (i in 0...5)
						{
							...
							black.scrollFactor.set(0.1, 0.1);
							black.visible = true;
							black.updateHitbox();
							blackback.add(black);
						}
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
			...
			if (PlayState.SONG.song.toLowerCase() == 'finale') //THIS WORKS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
		{
			//final scene then go back to storymenu
			FlxG.switchState(new EndingState());
		}
		
		    if (PlayState.SONG.song.toLowerCase() == 'tutorial') //THIS WORKS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
		{
			//final scene then go back to storymenu
			FlxG.switchState(new EndingState());
		}
	...
	override function beatHit()
	{
		...
		if (curBeat == 4 && curSong == 'lamprey-hole')
		{
			boyfriend.playAnim('hey', true);
			defaultCamZoom == 0.7;
			blackback.forEach(function(black:FlxSprite)
			{
				black.visible = false;
			});
		}
		
		if (curBeat == 164 && curSong == 'lamprey-hole')
		{
			defaultCamZoom = 0.9;
		}
		
		if (curBeat == 196 && curSong == 'lamprey-hole')
		{
			defaultCamZoom = 0.6;
			remove(boyfriend);
		}
